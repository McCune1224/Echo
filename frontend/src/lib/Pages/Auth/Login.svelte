        ...
        axios
            ...
            { 
                ...
                if("message" in response.data)
                    if(response.data["message"].includes("User does not exist"))
                    {
...
<div class={'text-white w-full h-full flex-1 ' + center} on:transitionend={(event) => SplitChildFromParentTransition(event)}>
    ...
    <form class='flex w-4/5 h-2/3' on:submit={(e) => OnClick(e)}>
        <div class="flex flex-col items-center justify-center w-full h-full">
            ...
            <button class={
                "w-full h-[60px] min-h-[60px] rounded-xl bg-blue-800 font-bold text-xl text-white " +
