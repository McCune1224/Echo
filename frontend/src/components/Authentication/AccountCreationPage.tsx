...
const AccountCreationPage = () => {
  ...
  var [disableInquiries, SetDisableInquiries] = React.useState(Array<boolean>(maximumDisableInquires)); 
  ...
  const AssignSubmitAffecter = (inputField: InputFields.InputField) => inputField.AffectSubmitButton(Callback, disableInquiries.length - 1); 
  ...
  var passwordInputField = new InputFields.PasswordInputField(SetPasswordInput); 
  AssignSubmitAffecter(passwordInputField);  

  var [usernameInput, SetUsernameInput] = useState(String);
  var usernameInputField = new InputFields.UsernameInputField(SetUsernameInput); 
  ...
  { 
    ...
    var userInfo = new UserInformation(emailInput, usernameInput, passwordInput); 
    axios.post("https://echo-backend-production.up.railway.app/user/register", {email:emailInput,username: usernameInput,password:passwordInput})
  ...
  { 
    if(disableSubmit)
      return <button disabled={true} className="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-5 rounded">Submit</button> 
    return <button className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-5 rounded">Submit</button>
  }
